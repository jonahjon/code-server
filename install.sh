...
Autoinstalls code-server on any macOS or Linux system.
...
v$VERSION of the macOS static archive on GitHub releases will be installed
...
If ran on any other Linux distro, v$VERSION of the linux static archive on GitHub
releases will be installed into /usr/local/lib/code-server-$VERSION.
...
main() {
  unset DRY_RUN STATIC STATIC_INSTALL_PREFIX SKIP
...
install_static() {
  ...
  echo "Installing v$VERSION static archive from GitHub releases."
  ...
  SKIP=1 "$sh_c" rm -Rf "$STATIC_INSTALL_PREFIX/code-server-$VERSION"
